<template>
  <div class="flex-1 overflow-y-auto">
    ...
    <div v-if="usersStore.loading" class="p-3 sm:p-4 space-y-3 sm:space-y-4">
    ...
    <div v-else-if="usersStore.filteredUsers.length === 0" class="p-4 sm:p-6 text-center">
      ...
      <p class="text-xs sm:text-sm text-gray-500">Try searching for users</p>
    </div>

    <div v-else>
      <!-- Online Users Section -->
      <div v-if="usersStore.onlineUsers.length > 0" class="mb-4 sm:mb-6">
        <div class="px-3 sm:px-4 py-2 bg-gray-50 border-b border-gray-200">
          <h3 class="text-xs sm:text-sm font-semibold text-gray-600 uppercase tracking-wide">
        ...
        <div>
          <button
            ...
            class="w-full p-3 sm:p-4 flex items-center hover:bg-gray-50 transition-colors group"
            :disabled="usersStore.loading"
          >
            <div class="relative flex-shrink-0">
              <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center text-white font-semibold text-sm sm:text-base" style="background: var(--primary-gradient)">
                {{ user.name.charAt(0).toUpperCase() }}
              </div>
              <!-- Online indicator -->
              <div class="absolute bottom-0 right-0 w-3 h-3 sm:w-4 sm:h-4 bg-green-500 rounded-full border-2 border-white"></div>
            </div>
            <div class="ml-2 sm:ml-3 flex-1 text-left min-w-0">
              <p class="font-semibold text-gray-900 group-hover:text-blue-600 transition-colors text-sm sm:text-base truncate">
                {{ user.name }}
              </p>
              <p class="text-xs sm:text-sm text-green-600 font-medium">Online now</p>
            </div>
      ...
      <div v-if="usersStore.offlineUsers.length > 0">
        ...
        <div>
          ...
          >
            <div class="relative flex-shrink-0">
              <div class="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-r from-gray-400 to-gray-500 rounded-full flex items-center justify-center text-white font-semibold text-sm sm:text-base">
                {{ user.name.charAt(0).toUpperCase() }}
              </div>
              <!-- Offline indicator -->
              <div class="absolute bottom-0 right-0 w-3 h-3 sm:w-4 sm:h-4 bg-gray-400 rounded-full border-2 border-white"></div>
            </div>
            <div class="ml-2 sm:ml-3 flex-1 text-left min-w-0">
              <p class="font-semibold text-gray-900 group-hover:text-blue-600 transition-colors text-sm sm:text-base truncate">
                {{ user.name }}
